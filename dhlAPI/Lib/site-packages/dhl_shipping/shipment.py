...
class Shipment:
    ...
    def get_shipment(self, dict_param):
        ...
        try:
            ...
            returned_data = dhl_process_obj.process_shipment(xml_formated_data_string)
            ...
            action_note = common_obj.in_dictionary_all('ActionNote', dict_response, 'nothing found')
            if 'Success' in action_note: # action_note is a list type
                return_status = True
                return_message = dhl_shipping.config.message_shipment_success
                # set pick up - if pickup provided
                if 'pickup_details' in dict_param:
                    if len(dict_param['pickup_details']) > 0:
                        request_for_pickup = True
                        pickup_returned_data = dhl_shipping.pickup.schedule_pickup(dict_param)
                        if pickup_returned_data['status']:
                            pickup_confirmation_number_temp = pickup_returned_data['pickup_confirmation_number']
                            pickup_confirmation_number = ''.join(map(str, pickup_confirmation_number_temp))
                ...
                airway_bill_number = common_obj.in_dictionary_all('AirwayBillNumber', dict_response) 
                airway_bill_number = ''.join(map(str, airway_bill_number))

                # get product short name
                product_short_name = common_obj.in_dictionary_all('ProductShortName', dict_response) 
                product_short_name = ''.join(map(str, product_short_name))

                # get package type
                package_type = common_obj.in_dictionary_all('PackageType', dict_response) 
                package_type = ''.join(map(str, package_type))

                # Chargeable Weight
                chargeable_weight = common_obj.in_dictionary_all('ChargeableWeight', dict_response) 
                chargeable_weight = ''.join(map(str, chargeable_weight))

                # Dimensional Weight
                dimensional_weight = common_obj.in_dictionary_all('DimensionalWeight', dict_response) 
                dimensional_weight = ''.join(map(str, dimensional_weight))

                # Insured Amount
                insured_amount = common_obj.in_dictionary_all('InsuredAmount', dict_response) 
                insured_amount = ''.join(map(str, insured_amount))

                # Shipment Date ie pickup or dropoff to dhl, not the date when its being created
                shipment_date = common_obj.in_dictionary_all('ShipmentDate', dict_response) 
                ...
                if 'optional_data' in dict_param.keys():
                    ...
                    if 'return_pdf_image_str' in optional_data.keys():
                        if optional_data['return_pdf_image_str']:
                            output_image_list = common_obj.in_dictionary_all('OutputImage', dict_response) 
                            output_pdf_image_str = ''.join(map(str, output_image_list))
                    ...
                    if 'return_pdf_file' in optional_data.keys():
                        if optional_data['return_pdf_file']:
                            create_pdf_data = self.create_pdf_awb(dict_param, dict_response, airway_bill_number)
            ...
            if dhl_shipping.dhl_response_flag:
                ...
                if 'dhl_response' in pickup_returned_data.keys():
                    return_dict.update({'dhl_response': {'dhl_shipment_response': dict_response, 'dhl_pickup_response': pickup_returned_data['dhl_response']}})
            ...
            return_dict.update(
                {
                    ...
                    'pickup_response': {
                        ...
                        'request_for_pickup': request_for_pickup, # True if user set up request for pickup else False
    ...
    def create_pdf_awb(self, dict_param, dict_response, airway_bill_number=''):
        ...
        if 'optional_data' in dict_param.keys():
            awb_pdf_file_path = ''
            awb_pdf_file_name = ''
            optional_data = dict_param['optional_data']  # awb file name and path to store
            ...
            if awb_pdf_file_path:
                ...
                file_data = base64.urlsafe_b64decode(output_image_str.encode('UTF-8'))

                # if folder not found then create one
                awb_pdf_file_path_name = os.path.join(awb_pdf_file_path, awb_pdf_file_name)
    ...
    def shipment_xml(self, dict_param):
        try:
            ...
            # Billing
            xml_str += '<Billing>'
            xml_str += '<ShipperAccountNumber>' + dhl_shipping.dhl_account_no + '</ShipperAccountNumber>'
            xml_str += '<ShippingPaymentType>' + dhl_shipping.shipping_payment_type + '</ShippingPaymentType>'
            xml_str += '<DutyPaymentType>' + dhl_shipping.duty_payment_type + '</DutyPaymentType>'
            xml_str += '</Billing>'
            # Billing ENDS

            # Consignee - ie the receiver - is to where the package will be send to - ie the to address
            xml_str += '<Consignee>'
            xml_str += '<CompanyName>' + addresses['to_company_name'] + '</CompanyName>'
            xml_str += '<AddressLine>' + addresses['to_address_line_one'] + '</AddressLine>' 
            xml_str += '<AddressLine>' + addresses['to_address_line_two'] + '</AddressLine>' # optionl
            ...
            xml_str += '<CountryName>' + addresses['to_country_name'] + '</CountryName>'
            xml_str += '<Contact>'
            xml_str += '<PersonName>' + addresses['to_name'] + '</PersonName>'
            xml_str += '<PhoneNumber>' + addresses['to_phone_no'] + '</PhoneNumber>'
            xml_str += '</Contact>'
            xml_str += '</Consignee>'
            # Consignee ENDS
            ...
            # Dutiable
            xml_str += '<Dutiable>'
            xml_str += '<DeclaredValue>' + str(package['declared_value']) + '</DeclaredValue>'
            xml_str += '<DeclaredCurrency>' + package['declared_currency'] + '</DeclaredCurrency>'  # currency code  -  3 letter abbriviation 
            xml_str += '</Dutiable>'
            # Dutiable ENDS

            # Reference
            xml_str += '<Reference>'
            xml_str += '<ReferenceID>' + str(package['reference_id']) + '</ReferenceID>'  
            ...
            xml_str += '<NumberOfPieces>' + str(len(pieces)) + '</NumberOfPieces>'  
            xml_str += '<CurrencyCode>' + package['declared_currency'] + '</CurrencyCode>'  # currency code  -  3 letter abbriviation 
            xml_str += '<Pieces>'
            piece_id = 1
            for piece in pieces:
                xml_str += '<Piece>'
                xml_str += '<PieceID>' + str(piece_id) + '</PieceID>'
                xml_str += '<PackageType>' + str(piece['package_type']) + '</PackageType>'  # DF|YP etc
                ...
                piece_id = piece_id + 1
            xml_str += '</Pieces>'
            xml_str += '<PackageType>' + str(package['package_type']) + '</PackageType>'  # DF|YP etc
            xml_str += '<Weight>' + str(package['total_weight']) + '</Weight>'
            xml_str += '<DimensionUnit>' + str(package['dimension_unit']) + '</DimensionUnit>'
            xml_str += '<WeightUnit>' + str(package['weight_unit']) + '</WeightUnit>'
            xml_str += '<GlobalProductCode>' + str(package['global_product_code']) + '</GlobalProductCode>'  # as P
            xml_str += '<LocalProductCode>' + str(package['local_product_code']) + '</LocalProductCode>'  # as P
            xml_str += '<DoorTo>DD</DoorTo>'
            xml_str += '<Date>' + str(package['shipment_date']) + '</Date>'  # Shipment date for when package(s) will be shipped (but usually current date)  ' + self.date_now.strftime("%Y-%m-%d") + '
            xml_str += '<Contents>' + str(package['content_description']) + '</Contents>'
            xml_str += '<IsDutiable>' + str(package['is_dutiable']) + '</IsDutiable>'  # Yes|No - Default is No
            xml_str += '<InsuredAmount>' + str(package['insured_amount']) + '</InsuredAmount>'  # Insured Amount (Required if Special Service of ‘I’ – Insurance is provided in request)
            xml_str += '</ShipmentDetails>'
            ...
            xml_str += '<AddressLine>' + addresses['from_address_line_one'] + '</AddressLine>'
            xml_str += '<AddressLine>' + addresses['from_address_line_two'] + '</AddressLine>'  # optional
            ...
            xml_str += '<CountryName>' + addresses['from_country_name'] + '</CountryName>'
            xml_str += '<Contact>'
            xml_str += '<PersonName>' + addresses['from_name'] + '</PersonName>'
            xml_str += '<PhoneNumber>' + addresses['from_phone_no'] + '</PhoneNumber>'
            xml_str += '</Contact>'
            xml_str += '</Shipper>'
            # Shipper ENDS
            ...
            if 'special_service_type' in package.keys():
                if package['special_service_type']:
                    ...
                    xml_str += '<SpecialServiceType>' + str(package['special_service_type']) + '</SpecialServiceType>'  # Insured Amount (Required if Special Service of ‘I’ – Insurance is provided in request)
                    xml_str += '</SpecialService>'
            # Special Service
